...
public class EntityCodeIdFillService {
    ...
    public <T extends BaseCodeEntity, ID> ID fill(Class<T> clazz, ID id, String code) {
        ...
        return (ID) ((EntityCodeDAO) EntityDAOManager.getDAO(clazz)).selectIdByCode(code).get();
    }
    ...
    public <T extends BaseCodeEntity> Map<String, Long> getCodeIdMap(Class<T> clazz, Collection<String> codes) {
        ...
        Map<String, Long> codeIdMap = ((EntityCodeDAOImpl) EntityDAOManager.getDAO(clazz)).selectForKeyValue("code, id", "code IN (:codes)", Map.of("codes", codes));
